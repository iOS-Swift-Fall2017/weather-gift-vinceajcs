...
class PageVC: UIPageViewController {
    ...
    func loadLocations() {
        guard let locationsEncoded = UserDefaults.standard.value(forKey: "locationsArray") as? Data else {
            print("Could not read in data from UserDefaults")
            return
        }
    ...
    func configurePageControl() {
        ...
        pageControl = UIPageControl(frame: CGRect(x: (view.frame.width - pageControlWidth) / 2,
                                                  y: safeHeight - pageControlHeight,
    ...
    func configureListButton() {
        ...
        listButton = UIButton(frame: CGRect(x: view.frame.width - barButtonWidth,
                                            y: safeHeight - barButtonHeight,
    ...
    func configureAboutButton() {
        ...
        let safeHeight = view.frame.height - view.safeAreaInsets.bottom
        aboutButton = UIButton(frame: CGRect(x: 8,
                                             y: (safeHeight - 5) - aboutButtonSize.height,
                                             width: aboutButtonSize.width,
                                             height: aboutButtonSize.height))
    ...
    @IBAction func unwindFromListVC(sender: UIStoryboardSegue) {
        ...
        setViewControllers([createDetailVC(forPage: currentPage)],
                           direction: .forward,
                           animated: false,
                           completion: nil)
    }
    ...
    @objc func pageControlPressed() {
       guard let currentViewController = self.viewControllers?[0] as? DetailVC else { return }
            ...
            if pageControl.currentPage < currentPage {
                setViewControllers([createDetailVC(forPage: pageControl.currentPage)],
                                   direction: .reverse,
                                   animated: true,
                                   completion: nil)
